...
class ChatRepositoryTest {
    ...
    @Test
    fun messagesAndPeersInsertedAndRetrieved() =
        runBlocking {
    ...
    @Test
    fun undeliveredForPeerReturnsOnlyPending() =
        runBlocking {
    ...
    @Test
    fun peersFlowEmitsSavedPeers() =
        runBlocking {
            val peer = PeerEntity("peerX", "Bob", 1L)
            repository.savePeer(peer)
            val list = repository.peers().first()
            assertEquals(listOf(peer), list)
        }
}
